...
bool Console::checkModel(string const &text)
{
    if (_model) {
	_out << "Replacing existing model" << endl;
	clearModel();
    ...
    string message;
    int status =  parse_bugs_string(text.c_str(), text.size(), _pvariables,
				    _pdata, _prelations, message);
    if (status != 0) {
	_err << endl << "Error parsing model text:" << endl << message << endl;
	//Tidy up
	delete _pdata; _pdata = 0;
	delete _prelations; _prelations = 0;
	if (_pvariables) {
